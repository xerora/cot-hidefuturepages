...
if(isset($cfg['allowpageexpire']) && $cfg['allowpageexpire']) {
	$expirepages_count = 0;
	if($cfg['trash_page']) {
		$sql_expiredpages = sed_sql_query("SELECT * FROM $db_pages WHERE page_expire<".(int)$sys['now_offset']);
	}
	else {
		$sql_expirepages = sed_sql_query("SELECT page_id, page_cat FROM $db_pages WHERE page_expire<".(int)$sys['now_offset']);
	}
